...
function shochatwmsg(isshow, msg) {
    ...
    resizeChatCodeCMT(CHAT_TOP, CHAT_LEFT, CHAT_WIDTH);
}
function reseth() {
...
function resizeChatCodeCMT(mtop, mleft, mwidth) {
    ...
    if (mwidth > 0) {
        ...
        $('#chatclientcmt  .navbar').width(mwidth);
    }
    ...
    if (CHAT_ISMOBILE == 1) {//move phai nâng cao
        ...
        if (mleft > 0) {
            fnMoveChat('#chatclientcmt .navbar', mtop, mleft, 0, 20);//move phai nâng cao
        } else {
            fnMoveChat('#chatclientcmt .navbar', mtop, left, 0, 20);//move phai nâng cao
        }
...
function RcmTrackingAjax(currentToken) {
    ...
    LoadCokieUserInfo();
    var type = 4;
    var pram = "cmd=trackcustomer&useragent=" + navigator.userAgent
        + "&token=" + currentToken
        + "&username=" + username
    ...
    if (CHAT_SITEID == 2 || CHAT_SITEID == 1) {
        url = "https://www.thegioididong.com/chat/home/track?" + pram;
    }
    if (CHAT_SITEID == 8) {
        url = "https://rtm.thegioididong.com/memtracking?" + pram;
    }
    var imgg = "<img src='" + url + "' style='height:0px;width:0px;'>";
    $('body').append($(imgg));

}
function getRandomInt5() {
...
function RcmTracking() {
    if (location.href.indexOf("cmpid=") > 0) {
        var campainid = "";
        var campainu =username;
        var url = location.href;
        try {
              campainid = url.match(/cmpid=([\w-]+)/)[1]; 
              campainu = url.match(/cmpu=([\w-]+)/)[1];
        } catch (e) {
...
function registerchatwithcommentnew(toppos, lefpos, widthpos, strname, intgender, strphone, stremail, commentid) {
...
socket.on('continue chat', function (data) {
    ...
    if (supporter == 'chatbot1' || supporter == 'chatbot2' || supporter == 'chatbot8') {
        CHAT_BOT = 1;
    } else {
        CHAT_BOT = 0;
    }
...
socket.on('chat deny', function () {
    CHAT_ISCONNECTED = false;
    shochatwmsg(1, 'Bạn đang chat với Tư vấn viên !');
...
socket.on("rdata", function (data) {
    ...
    if (obj.action == "CHAT") {
        ...
        if (CHAT_SITEID == 2) { 
            try {
                ga('send', 'event', 'chat', 'open chat', 'tracking');  
